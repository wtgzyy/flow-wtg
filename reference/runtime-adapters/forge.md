...
flow-wtg 默认仍然是纯 markdown：不需要 CLI、不需要安装运行时，也不绑定任何外部工具。Forge 是一个可选 runtime adapter，适合那些希望在 Claude Code 里防止 AI 跳阶段、漏产物、漏测试或漏 review 的项目。

## 它解决什么

flow-wtg 定义“应该怎么做”：阶段、prompt、template、产物和 review 规则。
...
Forge 不应该成为 flow-wtg 的依赖。

- 没装 Forge：flow-wtg 行为不变，继续按 `@flow-wtg/GO.md` 和 `prompts/*.md` 使用。
- 装了 Forge：AI 可以把 flow-wtg 的阶段、change-id、task-id 和风险信息写入 Forge state/routing，供 runtime guard 使用。
- Forge 缺失或不可用：必须 fail-open 到纯 markdown 流程，不应阻断 flow-wtg。
...
flow-wtg/templates/TASK.md
.claude/hooks/forge-pretool-guard.ps1
.claude/hooks/forge-session-audit.ps1
```

只有前 3 个存在时，说明项目使用 flow-wtg；同时存在 Forge hooks 时，说明可以启用 Forge runtime adapter。

## 阶段映射

| flow-wtg 阶段 | Forge 用途 |
|---|---|
...
Forge adapter 不要求 flow-wtg 改变产物格式。推荐只写一条轻量 routing event：
...
小改动、一次性脚本、纯讨论和非 Claude Code 环境，继续用纯 markdown flow-wtg 即可。
