...
> 提炼自 garden-skills `web-design-engineer`（基于 Anthropic Claude Design 系统提示词），针对 flow-wtg 上下文做了简化。
>
...
**问题**：iframe 嵌入的预览环境（flow-wtg 的大部分调试场景 + 在线预览工具都算）里，`scrollIntoView` 会触发外层 frame 的滚动，造成视觉错乱甚至把用户卷出预览区。
...
- 任务完成 → 回到 `@flow-wtg/prompts/4-dev.md` 步骤 4 self-review
