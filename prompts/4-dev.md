...
AI 不允许自行编造临时最小 TASK；缺字段必须反问用户或回到 `@flow-wtg/prompts/3-task.md` 生成正式 `TASK.md`。

若当前 task 涉及前端 / UI 文件（`.css` / `.tsx` / `.vue` / `.html` / `.svelte` / 设计 token / 用户可见文案），必须先确认 `.specs/<change-id>/UI-DESIGN.md` 存在。缺失时停止，回到 `@flow-wtg/prompts/2a-ui-design.md`。纯后端 / CLI / lib 任务才可跳过。
...
4. 若 `.specs/LESSONS.md` 不存在 → 用 `@flow-wtg/templates/LESSONS.md` 创建空骨架
...
1. 加载 `@.specs/<id>/UI-DESIGN.md`（必须存在；不存在 → 停下来要求先跑 `@flow-wtg/prompts/2a-ui-design.md`）
2. 加载 `@flow-wtg/reference/ui-anti-patterns.md`，按当前任务的关键词 grep 相关章节
3. 加载 `@flow-wtg/reference/frontend-engineer-rules.md`（**第 1 + 第 2 + 第 10 节必读**），其他节按输出类型按需读：
   - 任务是做交互原型 → 补读第 6.1
   - 任务是做幻灯片 / 演示 → 补读第 6.2
...
使用 `@flow-wtg/templates/SUMMARY.md` 模板，填到 `.specs/<change-id>/<task-id>-SUMMARY.md`。
...
1. **立即停手**——不要再写代码或跑工具
2. 用 `@flow-wtg/templates/PROGRESS.md` 写出 `.specs/<id>/<task-id>-PROGRESS.md`，重点填：
   - 已完成子步骤（勾选清单）
   - 当前正在做（一段话，恢复后能直接续上）
...
- 还有未完成任务 → 清窗，再次进入 `@flow-wtg/prompts/4-dev.md` 跑下一个
- 全部完成 → `@flow-wtg/prompts/5-test.md`
