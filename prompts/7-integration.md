...
3. 回到 `@flow-wtg/prompts/4-dev.md` 执行修复
4. 修完回到本步重跑
...
按 `@flow-wtg/templates/LESSONS.md` 末尾的「提名条件」筛选：
...
📋 归档操作已导致以下文件变更：
   ...
   修改：
     - STATE.md（last_archive 更新）
     - .specs/CHANGELOG.md（新增一行）

   新增：
     - .specs/archive/<YYYY-MM-DD>-<change-id>/（归档目录）

   删除：
     - .specs/<change-id>/（原目录已移入 archive）

是否提交 git？
  1. 提交（AI 自动 add + commit）
  2. 不提交（我稍后手动处理）

请选 1/2。
```

- **选 1**：执行 `git add` + `git commit`，提交信息格式：`chore(archive): <change-id> 归档`
- **选 2**：跳过，提醒用户稍后自行处理
- **禁止**在用户未确认的情况下执行 git 操作
...
- **Git 操作必须用户确认**：归档后的 git add/commit 必须询问开发者，禁止自动提交
...
- 此 CHANGE 完成 → 等下一个 CHANGE，回到 `@flow-wtg/prompts/0-change.md`
