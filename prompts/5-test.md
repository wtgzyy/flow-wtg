...
> 每轮按项目类型可裁剪。本 prompt 决定**做哪几轮**；具体怎么做查 `@flow-wtg/reference/test-pyramid.md`。
...
- 各任务的 `*-SUMMARY.md`
- 已存在的测试代码
- `@flow-wtg/reference/test-pyramid.md`（5 轮的工具 / 标准 / 清单）
...
- `.specs/<change-id>/TEST.md`（用 `@flow-wtg/templates/TEST.md` 模板，含 5 轮报告段）
...
- 任何轮次发现 🔴 问题 → 回到 `@flow-wtg/prompts/4-dev.md` 修复（产 fix 任务）
- 全部通过 → `@flow-wtg/prompts/6-review.md`（review 阶段会再检查 5 轮都做了）
