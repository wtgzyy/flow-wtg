...
> 在已存在的项目（brownfield）里第一次用 flow-wtg 时，**先跑这个**。
...
- 进入 0-change 时检测 `CONTEXT.md` 不存在或字段缺失（见 GO.md 入场检测段）
...
flow-wtg 用 CONTEXT.md 作为单一源。请选择：
...
flow-wtg 需要一份 CONTEXT.md 给 AI 用。请选择：
...
- 跑完 → 用户继续原本意图（如 `@flow-wtg/GO.md` + 一句新需求）
