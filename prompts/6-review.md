...
- `@flow-wtg/reference/ui-anti-patterns.md`（如是前端项目）

## 你的职责

使用 `@flow-wtg/templates/REVIEW.md` 模板分三轮审查（后端 / lib 项目跳过第三轮）。
...
**输出必须包含**该工具要求的四要素（也是 flow-wtg 下游认的格式）：
...
逐项对照 `@flow-wtg/reference/ui-anti-patterns.md` 的"强制禁忌"段：
...
- 有 Critical 待修 → `@flow-wtg/prompts/4-dev.md`（执行 fix 任务）
- 全部通过或人工接受 → `@flow-wtg/prompts/7-integration.md`
