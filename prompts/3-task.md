...
- 缺 `REQUIREMENT.md`：停止，回到 `@flow-wtg/prompts/1-requirement.md`。
- 缺 `DESIGN.md`：停止，回到 `@flow-wtg/prompts/2-design.md`；MVP 模式也必须生成 `DESIGN-lite`。
- 前端 / UI 项目缺 `UI-DESIGN.md`：停止，回到 `@flow-wtg/prompts/2a-ui-design.md`。纯后端 / CLI / lib 项目才可跳过。
...
使用 `@flow-wtg/templates/TASK.md` 模板产出**原子任务列表**。
...
`@flow-wtg/prompts/4-dev.md`（按波次逐个执行）
