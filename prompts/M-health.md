...
> **触发方式**：`@flow-wtg/GO.md` + `健康检查 / health / 体检 / 技术债扫描 / 巡检`
...
随机抽 5 个最近改动频繁的模块（用 `git log --pretty=format: --name-only | sort | uniq -c | sort -rn | head -5`），每个按 6 维（R1~R6，见 `@flow-wtg/prompts/6-review.md` 第 2.1 节）打分。

#### 3.2 测试 6 维抽样

随机抽 5 个测试文件，按 T1~T6（见 `@flow-wtg/prompts/5-test.md` 第 1.4 节）打分。
...
- 有 🔴 Critical → 用户选「现在修」 → `@flow-wtg/prompts/0-change.md` 开 health-fix change
