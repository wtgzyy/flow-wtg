...
- **R1.5 · 重启协议（清窗前后必须严格执行）**
  ...
  **清窗前** AI 必须依次完成：
  1. 把当前任务的中间状态写入 `.specs/<id>/<task-id>-PROGRESS.md`（用 `@flow-wtg/templates/PROGRESS.md`）。必须包含：已完成子步骤 / 当前正在做 / **已排除方案及理由** / 待确认假设。
...
- **R1.9 · 工件加载预算（任何阶段首轮强制）**
  ...
  flow-wtg 的文件按加载策略分三类：
  | 类型 | 路径 | 长度 | 加载方式 |
  |---|---|---|---|
  | SPEC | `.specs/<id>/*.md` | 通常 < 200 行 | 整读 OK |
  | REFERENCE | `flow-wtg/reference/*.md` | 75~470 行 | **禁止默认整读**，按节 grep / read offset |
  ...
  5. 完整加载策略表见 `@flow-wtg/GO.md` 顶部「红线·Token 预算」段
...
- **R4.5 · Schema 变更必伴随迁移文件（强制）**
  ...
  详细操作步骤见 `@flow-wtg/prompts/4-dev.md` 第 1.7 节。

- **R4.6 · 破坏性变更高门槛（强制）**

  AI 在 4-dev 阶段命中下面**任一**条件时，禁止直接动手，必须先走「破坏性变更协议」（详见 `@flow-wtg/prompts/4-dev.md` 第 1.8 节）：
...
- **R5.4** TEST.md 必须声明 5 轮金字塔（功能 / 性能 / 安全 / 兼容 / 可观测）的状态。跳过任意一轮必须给理由（"暂时跳过"不算理由）。详见 `@flow-wtg/reference/test-pyramid.md`。
