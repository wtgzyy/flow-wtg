...
一套 AI 编程流程文档。我把自己写项目的方法抽成可复用的 markdown 包。

设计思路来源于最近用过的几个开源项目——OpenSpec、GSD、claude-task-master、superpowers、gstack、skills——提取其中有用部分，按自己的工作流重新编排。去掉品牌和 CLI 依赖，只保留 markdown。

装一次，不绑定任何上游版本。

## 为什么是纯 markdown

经验表明：工具只要需要"安装"，就必然有版本问题、依赖冲突、breaking change。flow-wtg 反其道而行——它不是工具，是一组文件。clone 到项目根目录就能用，没有运行时。

- 不需要 `npm install`、`pip`、CLI
- 任何支持 `@` 引用文件的 AI IDE（Windsurf / Claude Code / Cursor / Copilot / Codex / Gemini / Cline）都能用
- 所有规则都在文件里，想改就改

缺点是手动操作多了一点，好处是不会被工具更新坑到。

---

## 安装（30 秒）

把 `flow-wtg/` 文件夹复制到你的新项目根目录：
...
```
请先读取并严格遵守：
@flow-wtg/METHODOLOGY.md
@flow-wtg/RULES.md
@flow-wtg/prompts/0-change.md
...
copy flow-wtg\prompts\*.md .windsurf\workflows\
```
...
cp flow-wtg/prompts/0-change.md .claude/skills/flow-change/SKILL.md
# 其他阶段同理
```

或者更简单——直接在会话里 `@flow-wtg/...` 引用。
...
都支持 `@` 引用文件，直接按"通用方式"操作。

---

## 降低输入摩擦（每次 @ 太烦怎么办）

### 终极极简（每个 IDE 通用，1 个 @，不用想 ID）

**任何 IDE、任何 AI、任何会话**，只需要：
...
`GO.md` 是**统一入口**：
- 自动判断你想做哪个阶段（新需求 / 继续 / 执行任务 / 审查 / 测试 / 集成）
- 自动生成 `change-id`（你不用想名字）
- 自动加载该阶段需要的所有工件
- 自动反问澄清

**所有 IDE 通用**——`@文件` 是所有 AI 工具的最低公分母。

> 配合方式 B（全局注入 `SYSTEM.md`），AI 在路由的同时也守规则。这是最干净的组合。

> 如果你的 IDE 支持斜杠命令（Windsurf / Continue），还可以把 `GO.md` 复制为 `/go` 命令，连 1 个 `@` 都省了：
> ```powershell
> mkdir -Force .windsurf\workflows
> copy flow-wtg\GO.md .windsurf\workflows\go.md
> copy flow-wtg\prompts\*.md .windsurf\workflows\
> ```
> 之后输入 `/go 设计陪诊网站` 即可。
...
下面三种是更细粒度的传统方式，如果你想直接控制每一步、不用自动路由可看：
...
| Aider | `.aider.conf.yml` 加 `read: [flow-wtg/SYSTEM.md]` |
...
不想配任何东西、就想 ctrl+c ctrl+v？把下面这些保存到你的笔记软件，每次直接拷：
...
**1-requirement**：
```
@flow-wtg/prompts/1-requirement.md @.specs/<id>/CHANGE.md
按 1-requirement 流程出 REQUIREMENT.md 和 .specs/CONTEXT.md。
```

**2-design**：
```
@flow-wtg/prompts/2-design.md @.specs/<id>/REQUIREMENT.md @.specs/CONTEXT.md
按 2-design 流程出 DESIGN.md。
```

**3-task**：
```
@flow-wtg/prompts/3-task.md @.specs/<id>/REQUIREMENT.md @.specs/<id>/DESIGN.md @.specs/CONTEXT.md
按 3-task 流程出 TASK.md。
```

**4-dev**（每任务一次）：
```
@flow-wtg/prompts/4-dev.md @.specs/<id>/TASK.md @.specs/CONTEXT.md @.specs/LESSONS.md
执行 task <T01>。
```

**5-test / 6-review / 7-integration** 类似，引用对应 prompt + 已有产物即可。

> 配合**方式 B**，可以把每条开头的 `@flow-wtg/prompts/...` 省掉。
...
> 推荐：**A + B 一起上**——配了斜杠命令同时把 SYSTEM.md 注入全局规则，斜杠命令不工作时回退到 `@` 也少一个文件。
...
- 但 BMAD 靠 `bmad-correct-course` 在 sprint 中修订，flow-wtg 靠 `A-evolve` 在多个 change 后批量同步——后者更贴合 change-driven 哲学
...
| `prompts/I-intel-scan.md` | **老项目入场扫描**：检测 AI 上下文文档 + 扫代码生成/更新 `.specs/CONTEXT.md` | ✅ 完全独立 | brownfield 项目首次使用 flow-wtg 必跑；老项目架构偏移后重扫 |
| `prompts/A-architect.md` | **项目级架构梳理**：建立/重构 `ARCHITECTURE.md`，含模块图 + 依赖规则 + ADR 列表 + 跨模块契约 + 容量边界 | ✅ 完全独立 | 项目里程碑后架构梳理；接手陌生项目后结构化架构理解；ADR 定期重审 |
| `prompts/A-evolve.md` | **架构增量同步**：扫近期归档 change 的 DESIGN § 9，逐项 review 后 patch CONTEXT.md / ARCHITECTURE.md | ✅ 完全独立 | 每月/每季批量同步一次；里程碑发布后凝固架构决策 |
...
### 可选外部扩展（按需安装，flow-wtg 自动检测并优先使用）

| 扩展 | 维度 | 角色 | flow-wtg 何处优先调用 | 何时受益 |
|---|---|---|---|---|
...
- **flow-wtg 编排流程**：定义在哪个阶段做哪类决策（0-change 选调性 / 2-design 选栈 / 2a 深化视觉 / 4-dev 落地 / 5-test 测试 / 6-review 审 / M-health 巡检）
...
**都没装 flow-wtg 也能跑**——内置 `reference/*` 作为基线，6 维代码诊断/6 维测试诊断/9 调性/反 AI-slop 清单/冗余 grep fallback 都有内置回退。装上后**质量明显提升**（按 brooks-lint benchmark：带书本引用的代码 review 发现率 100% vs 不带 16%；字面冗余扫描 jscpd 的 recall 远高于 AI grep fallback）。

---

### 可选运行时门禁：Forge

flow-wtg 默认仍然是纯 markdown、无运行时。如果你在 Claude Code 里经常遇到 AI 跳过阶段、漏写产物、漏测试或漏 review，可以额外接入 Forge 作为可选 runtime adapter。

Forge 的角色不是替代 flow-wtg，而是读取 flow-wtg 的阶段/change-id/task-id，通过 Claude Code hooks、routing log、health check 和 smoke test 做运行时门禁。没装 Forge 时，flow-wtg 行为完全不变。
...
> **核心一条**：除了 `PROGRESS.md` 和 `STATE.md` 这两种和流程绑定的临时/状态文件，其它每一个 prompt 与 template 都设计成可独立使用，不要求你必须全套上车。

---

## Token 成本表（提前知道价格）

> flow-wtg 的设计是 **token 多花、单窗压力小、产物有外溢价值**。下表帮你按场景选挡位。
> 完整估算逻辑见 `@flow-wtg/GO.md` 的「Token 预算估计」段。
...
| **flow-wtg 极简模式** | ~205k - 445k | 低（每 task fresh ctx） | REQUIREMENT / DESIGN / TASK / SUMMARY × N / TEST / REVIEW + LESSONS（UI 项目另含 UI-DESIGN） | 中等 feature / 个人长期项目 |
| **flow-wtg 完整模式** | ~250k - 530k | 低 | 8 个 .md + LESSONS + history | 团队项目 / PR 必走 / 长期维护 |
| **flow-wtg + brooks-lint 完整** | ~280k - 600k | 低 | 同上 + 书本引用 review | 严肃工程 / 跨人交接 / 高质量门 |

### 怎么选

1. **改 < 30 行 / 一次性 / 简单 bugfix** → 跳过 flow-wtg，让 AI 直接改
2. **改 30-100 行 / 个人项目** → 走 7 个原生 skill（brainstorm + TDD + review 三件套就够）
3. **改 100-500 行 / 想要可追溯产物** → flow-wtg 极简模式，或 flow-wtg 单点调 6-review
4. **改 500+ 行 / 团队 / 长期** → flow-wtg 完整 + brooks-lint，**多花的 token 沉淀为团队资产**

### 为什么 flow-wtg token 多

主要花在三处（按贡献排序）：

- **40%** · 每个 task 一个 fresh context，重新加载 prompt + spec（这是 R1.4 的核心设计——降低单窗压力，避免 50k+ 的窗内崩盘/"打转"）
...
**剩下 15% 是 prompt 文件本身长**——已被 R1.9 工件加载预算 + reference 按节读 + R1.3 引用历史用 `@路径` 等机制压到底了。

---

## 老项目（brownfield）安全护栏总览

> flow-wtg 的两个高频担忧：**AI 不按既有架构开发** + **AI 乱删乱改不相关代码**。下表是每条护栏拦的具体事故。

| 编号 | 护栏 | 文件 | 拦的事故 |
|---|---|---|---|
| **B1** | 入场扫描自动生成 CONTEXT.md | `prompts/I-intel-scan.md` + `GO.md` 第三步 | AI 不知道项目栈/命名风格/既有抽象 → 写出格格不入的代码 |
...
| 项目 | 首次 | 之后每个 change |
|---|---|---|
| 5 万行老项目首次跑 flow-wtg | +15-30k（intel-scan）= ~265-560k | ~250-530k（与 greenfield 同） |
